...
# # ----------------------------------------------------#
# def channel_attention(tensor, pooling_type='attention_avg'):  # tensor = [B,C,H,W]
#     """
...
#     global_p2m = 1.5 * (F.max_pool2d(tensor2, kernel_size=shape[2:]))
#
...
#22222222222222222222222222222222222222222222222222222222222gpt
import torch
import torch.nn.functional as F
...
# attention fusion strategy, average based on weight maps
def attention_fusion_strategy(tensor1, tensor2, weight=0.5, channel_type='attention_max', spatial_type='mean'):
    print(f"DEBUG: 正在使用权重 weight = {weight}")
    # 通道加权融合
    f_channel = channel_fusion(tensor1, tensor2, weight, channel_type)
    # 空间加权融合
    f_spatial = spatial_fusion(tensor1, tensor2, weight)
    # 最终融合结果
    tensor_f = (f_channel + f_spatial) / 2
    return tensor_f

def channel_fusion(tensor1, tensor2, weight, pooling_type='attention_max'):
    shape = tensor1.size()
    global_p1 = channel_attention(tensor1, pooling_type)
    global_p2 = channel_attention(tensor2, pooling_type)

    # 引入 alpha 调节：给 global_p1 乘上 alpha，给 global_p2 乘上 (1 - alpha)
    # 这样即便计算出的 Attention 相同，最终权重也会受 alpha 影响
    w1_val = global_p1 * weight
    w2_val = global_p2 * (1 - weight)

    global_p_w1 = w1_val / (w1_val + w2_val + 1e-6)
    global_p_w2 = w2_val / (w1_val + w2_val + 1e-6)

    global_p_w1 = global_p_w1.expand(-1, -1, shape[2], shape[3])
    global_p_w2 = global_p_w2.expand(-1, -1, shape[2], shape[3])

    return global_p_w1 * tensor1 + global_p_w2 * tensor2



def spatial_fusion(tensor1, tensor2, weight):
    mask1, _ = tensor1.abs().max(dim=1, keepdim=True)
    mask2, _ = tensor2.abs().max(dim=1, keepdim=True)

    # 通过 alpha 调整显著性度量的阈值
    # 如果 alpha 很大，mask1 只需要很小就能胜过 mask2
    # 公式：alpha * mask1 > (1 - alpha) * mask2
    hard_mask = (weight * mask1 > (1 - weight) * mask2).float()

    tensor_f = hard_mask * tensor1 + (1 - hard_mask) * tensor2
    return tensor_f
