...
class denselayer(nn.Module):
    def __init__(self, num_channels, growth, kernel_size, stride):
        super(denselayer, self).__init__()
        self.conv_b = ConvBlock(num_channels, growth, kernel_size, stride)#in num_channels = 16, out growth = 64
        # self.sobel = juanji_sobelxy(num_channels)
        self.sobel = juanji_sobelxy(num_channels, kernel_size=3, padding=1, stride=1, dilation=1, groups=1)
        self.sobel_conv = nn.Conv2d(num_channels, growth, 1, 1, 0)

        self.down = nn.Conv2d(growth*2, growth, 3, 1, 1)
        # self.down1 = nn.Conv2d(64, 64,  3, 1, 1)
    def forward(self, x):
        # print("x", x.shape)
        x1 = self.conv_b(x)
        x2 = self.sobel(x)
...
class CNN_Encoder(nn.Module):
    def __init__(self, input_nc=1, kernel_size=3, stride=1):
        ...
        self.DenseBlock1 = DenseBlock(64)
        self.DenseBlock2 = DenseBlock(112)
        self.DenseBlock3 = DenseBlock(160)
        self.DenseBlock4 = DenseBlock(208)

# ---------------我的test2-----------------
    def forward(self, input):
        # print("input的通道数为:", input.shape)
        x = self.conv0(input)
        x_1 = self.ECB1_0(x)
        x_1 = self.DenseBlock1(x_1)
        # print("x_1的通道数为:", x_1.shape)

        x_2 = self.ECB2_0(self.pool(x_1))
        x_2 = self.DenseBlock2(x_2)
        # print("x_2的通道数为:", x_2.shape)

        x_3 = self.ECB3_0(self.pool(x_2))
        x_3 = self.DenseBlock3(x_3)
        # print("x_3的通道数为:", x_3.shape)

        x_4 = self.ECB4_0(self.pool(x_3))
        x_4 = self.DenseBlock4(x_4)
        # print("x_4的通道数为:", x_4.shape)
...
class Nest_Decoder(nn.Module):
    ...
    def forward(self, f_en):
        ...
        x1_1 = self.Lrelu(self.bn1(self.DCB1_1(torch.cat([f_en[0], self.up(f_en[1])], dim=1))))
        # x1_1 = self.DCB1_1(torch.cat([f_en[0], self.up(f_en[1])], dim=1))


        x2_1 = self.Lrelu(self.bn2(self.DCB2_1(torch.cat([f_en[1], self.up(f_en[2])], dim=1))))

        x3_1 = self.Lrelu(self.bn3(self.DCB3_1(torch.cat([f_en[2], self.up(f_en[3])], dim=1))))

        x1_2 = self.Lrelu(self.bn4(self.DCB1_2(torch.cat([f_en[0], x1_1, self.up(x2_1)], dim=1))))

        x2_2 = self.Lrelu(self.bn5(self.DCB2_2(torch.cat([f_en[1], x2_1, self.up(x3_1)], dim=1))))

        # print("222222222222")
        x1_3 = self.Lrelu(self.bn6(self.DCB1_3(torch.cat([f_en[0], x1_1, x1_2, self.up(x2_2)], dim=1))))
...
class Decoder_eval(nn.Module):
    ...
    def forward(self, f_en):
        x1_1 = self.Lrelu(self.bn1(self.DCB1_1(torch.cat([f_en[0], self.up_eval(f_en[0], f_en[1])], dim=1))))
        # print("x1.1",x1_1.shape)
        x2_1 = self.Lrelu(self.bn2(self.DCB2_1(torch.cat([f_en[1], self.up_eval(f_en[1], f_en[2])], dim=1))))
        x3_1 = self.Lrelu(self.bn3(self.DCB3_1(torch.cat([f_en[2], self.up_eval(f_en[2], f_en[3])], dim=1))))
        x1_2 = self.Lrelu(self.bn4(self.DCB1_2(torch.cat([f_en[0], x1_1, self.up_eval(f_en[0], x2_1)], dim=1))))
        x2_2 = self.Lrelu(self.bn5(self.DCB2_2(torch.cat([f_en[1], x2_1, self.up_eval(f_en[1], x3_1)], dim=1))))
        x1_3 = self.Lrelu(self.bn6(self.DCB1_3(torch.cat([f_en[0], x1_1, x1_2, self.up_eval(f_en[0], x2_2)], dim=1))))
        if self.deepsupervision:
            output1 = self.conv1(x1_1)
            output2 = self.conv2(x1_2)
            output3 = self.conv3(x1_3)
...
class DenseBlock(nn.Module):
    def __init__(self, in_channels, growth_rate=32):
        ...
        self.conv3 = nn.Conv2d(in_channels + 2 * growth_rate, growth_rate, kernel_size=3, padding=1, bias=True)
        self.conv4 = nn.Conv2d(in_channels + 3 * growth_rate, growth_rate, kernel_size=3, padding=1, bias=True)
        self.conv5 = nn.Conv2d(in_channels + 4 * growth_rate, in_channels, kernel_size=3, padding=1, bias=True)
