...
def attention_fusion_strategy(tensor1, tensor2, channel_type='attention_max', spatial_type='mean'):
    # 通道加权融合
    f_channel = channel_fusion(tensor1, tensor2, channel_type)
    # 空间加权融合
    f_spatial = spatial_fusion(tensor1, tensor2, spatial_type)
    # 最终融合结果
    tensor_f = (f_channel + f_spatial) / 2
    return tensor_f
...
#
#     # 计算空间加权
#     spatial1 = spatial_attention(tensor1, spatial_type)  # [B,1,H,W]
#     spatial2 = spatial_attention(tensor2, spatial_type)  # [B,1,H,W]
#
#     # 使用softmax进行权重归一化
#     spatial_w1 = torch.exp(spatial1) / (torch.exp(spatial1) + torch.exp(spatial2) + EPSILON)
#     spatial_w2 = torch.exp(spatial2) / (torch.exp(spatial1) + torch.exp(spatial2) + EPSILON)
#
#     # 将权重扩展到通道维度
#     spatial_w1 = spatial_w1.repeat(1, shape[1], 1, 1)  # [B,C,H,W]
#     spatial_w2 = spatial_w2.repeat(1, shape[1], 1, 1)  # [B,C,H,W]
#
#     # 对原始特征图进行加权融合
#     tensor_f = spatial_w1 * tensor1 + spatial_w2 * tensor2
#     return tensor_f
def spatial_fusion(tensor1, tensor2, spatial_type='max'):
    # print(">>> DEBUG: 正在运行修改后的 Max 融合逻辑！")
    # 获取两张特征图在每个像素位置的最大响应值
    # 假设 tensor1 是红外，tensor2 是可见光

    # 1. 计算每个位置的显著性度量（例如 L1 范数或直接用最大值）
    mask1, _ = tensor1.abs().max(dim=1, keepdim=True)
    mask2, _ = tensor2.abs().max(dim=1, keepdim=True)

    # 2. 生成二值掩码：哪个响应大就选哪个
    # 这就是所谓的像素级硬最大值融合
    hard_mask = (mask1 > mask2).float()
...
#     if spatial_type == 'mean':
#         # 计算每个位置的均值
#         spatial = tensor.mean(dim=1, keepdim=True)  # [B,1,H,W]
#     elif spatial_type == 'sum':
#         # 计算每个位置的和
#         spatial = tensor.sum(dim=1, keepdim=True)
#     elif spatial_type == 'max':
#         # 计算每个位置的最大值
#         spatial, _ = tensor.max(dim=1, keepdim=True)  # [B,1,H,W]
#     else:
#         raise ValueError(f"Unsupported spatial type: {spatial_type}")
#     return spatial
